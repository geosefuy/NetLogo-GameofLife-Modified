...
to go-anjelo
  ...
  ask patches
    [ ifelse live-neighbors >= 6 and living = 1
      [ cell-infect ]
      [ set live-neighbors count neighbors with [living = 2] ] ]
  ask patches
    [ ifelse live-neighbors >= 2 and living = 1
      [ cell-infect ]
  ;; infect to death
      [ set live-neighbors count neighbors with [living = 2] ] ]
  ask patches
    [ if live-neighbors >= 3 and living = 2
      [ cell-death ]  ]
  ;; reproduction
  ask patches
      [ set live-neighbors count neighbors with [living = 1] ]
