...
to go
  ...
  ask patches
    [ ifelse live-neighbors >= 6 and pcolor = green
      [ cell-infect ]
      [ set live-neighbors count neighbors with [pcolor = red] ] ]
  ask patches
    [ ifelse live-neighbors >= 2 and pcolor = green
      [ cell-infect ]
  ;; infect to death
      [ set live-neighbors count neighbors with [pcolor = red] ] ]
  ask patches
    [ if live-neighbors >= 2 and pcolor = red
      [ cell-death ]  ]
  ;; reproduction
  ask patches
      [ set live-neighbors count neighbors with [pcolor = green] ]
  ask patches
    [ if live-neighbors mod 2 = 0 and pcolor = white
      [ cell-normal ] ]
  tick
end
